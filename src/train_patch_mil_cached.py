...
def evaluate(model,loader,device):
    model.eval(); p=[]; y=[]
    for e,v,_,sid,_,lab in tqdm(loader,desc='eval',leave=False):
        e=e.to(device); v=v.to(device); sid=sid.to(device)
        logits=model.forward_from_tokens(e,v,sid); p.extend(torch.sigmoid(logits).cpu().numpy().tolist()); y.extend(lab.numpy().tolist())
...
def train():
    ...
    model=PatchMILClassifier(hidden_size=1024,attn_hidden=pm.attention_pooling_hidden,head_dims=list(pm.head_dims),dropout=pm.dropout,context_layers=getattr(pm,'context_layers',0),context_heads=getattr(pm,'context_heads',8),encoder_type='torchxrayvision',skip_encoder_build=True).to(device)
    ...
    for epoch in range(cfg.training.epochs):
        model.train();
        for e,v,c,sid,rel,y in tqdm(ls['train'],desc=f'Epoch {epoch+1}',leave=False):
            e=e.to(device); v=v.to(device); c=c.to(device); sid=sid.to(device); rel=rel.to(device); y=y.to(device).float(); opt.zero_grad(set_to_none=True)
            logits,attn,critical,inst=model.forward_with_attention_and_instance_from_tokens(e,v,sid); main=loss_fn(logits,y); cont=model.compute_contiguity_loss(attn,c); safe=attn.clamp_min(1e-12); ent=-(safe*torch.log(safe)).sum(1).mean(); aux=loss_fn(critical,y)
