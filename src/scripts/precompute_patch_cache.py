...
def build_cache(cfg,model,device,split,bbox_map,out_root):
    ...
    ds=PatchMILDataset(str(cfg.paths.processed_root),split,pm.canvas_size,pm.patch_size,pm.stride,pm.coverage_threshold,
                       ...
                       [dict(s) for s in pm.scales] if use_ms else None,getattr(pm,'multiscale_output_size',64),getattr(pm,'max_patches_per_scale',50),getattr(pm,'mask_prior_type','distance'))
    ...
    with torch.no_grad():
        for idx in tqdm(range(len(ds)),desc=f'Cache {split}'):
            ...
            patches,valid,coords,scale_ids,mask_prior,_=ds[idx]
            ...
            np.savez_compressed(out,embeddings=emb,coords=coords.numpy().astype(np.float32),scale_ids=scale_ids.numpy().astype(np.int64),mask_prior=mask_prior.numpy().astype(np.float32),label=np.array(label),relevance=rel)
