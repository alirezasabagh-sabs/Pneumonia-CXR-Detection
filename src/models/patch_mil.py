...
class AttentionPooling(nn.Module):
    ...
    def forward(self, patch_tokens, validity=None, mask_prior=None, return_weights=False):
        scores = self.attn(patch_tokens)
        if validity is not None:
            valid = validity > 0
            neg_inf = torch.finfo(scores.dtype).min
            scores = scores + torch.where(valid.unsqueeze(-1), torch.zeros_like(scores), torch.full_like(scores, neg_inf))
        if mask_prior is not None:
            if self.hard_filter_threshold is not None:
                keep = mask_prior >= self.hard_filter_threshold
                if validity is not None:
                    keep = keep & (validity > 0)
...
class PatchMILClassifier(nn.Module):
    def __init__(self, hidden_size=None, attn_hidden=128, head_dims=None, dropout=0.3,
                 ...
                 txrv_input_size=128, mask_strength=0.0, hard_filter_threshold=None,
                 skip_encoder_build=False):
        super().__init__()
        ...
        self.attention_pool = AttentionPooling(
            self.hidden_size, attn_hidden, mask_strength=mask_strength,
    ...
    def forward(self, patches, validity, scale_ids=None, mask_prior=None):
        tokens = self._apply_context(self._encode_patches(patches), validity)
        return self.head(self.attention_pool(tokens, validity, mask_prior)).squeeze(-1)
    def forward_with_attention(self, patches, validity, scale_ids=None, mask_prior=None):
        tokens = self._apply_context(self._encode_patches(patches), validity)
        pooled, weights = self.attention_pool(tokens, validity, mask_prior, return_weights=True)
        return self.head(pooled).squeeze(-1), weights
    def forward_from_tokens(self, tokens, validity, scale_ids=None, mask_prior=None):
        tokens = self._apply_context(tokens, validity)
        pooled = self.attention_pool(tokens, validity, mask_prior)
        return self.head(pooled).squeeze(-1)
    def forward_with_attention_from_tokens(self, tokens, validity, scale_ids=None, mask_prior=None):
        tokens = self._apply_context(tokens, validity)
        pooled, weights = self.attention_pool(tokens, validity, mask_prior, return_weights=True)
        return self.head(pooled).squeeze(-1), weights
    def forward_with_attention_and_instance_from_tokens(self, tokens, validity, scale_ids=None, mask_prior=None):
        tokens = self._apply_context(tokens, validity)
        pooled, weights = self.attention_pool(tokens, validity, mask_prior, return_weights=True)
